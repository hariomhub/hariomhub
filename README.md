## Hey there! I'm Hariom 👋
Aspiring Software Developer | Problem Solver
<p>
  <a href="https://linkedin.com/in/hariomkv"><img src="https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="mailto:hariomkumarkjh55@gmail.com"><img src="https://img.shields.io/badge/Email-red?logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://codolio.com/profile/hariom_k"><img src="https://img.shields.io/badge/Codolio-000?logo=code&logoColor=white" alt="Codolio" /></a>
  <a href="https://github.com/hariomhub"><img src="https://komarev.com/ghpvc/?username=hariomkumar&color=brightgreen" alt="Profile views" /></a>
</p>

### About Me
- 🎓 Studying B.Tech(ECE) at **MSIT**, Delhi 
- 🧩 Improving problem-solving proficiency through Competitive Programming.
- 🔭 Previously interned at WS Mobility, Ex-Trainee at GeeksForGeeks, Ex-SME at Chegg, Mentor at HackWithIndia
- 🚀 Passionate about building scalable and impactful web applications.
- 🌱 Currently exploring **Next.js, TypeScript** and **Machine Devops** 
- 💬 Ask me about **DSA, CP, Java, MERN Stack, SQL**

### 🛠️ Tech Stack
💻  `Java` | `JavaScript` | `Python` | `SQL` </br>
🌐 `HTML5` | `CSS` | `TailwindCSS` | `React.js` | `Node.js` | `Express.js` | `Redux` </br> 
🛢 `MongoDB` | `MySQL`  
⚙️ `Git` | `GitHub` | `Postman` | `VS Code` | `Vercel` | `Render` | `Firebase`

### 🚀 My Projects

### 🏆 Achievements

### 📈 GitHub Stats  

<p align="center">
  <img width="50%" src="https://github-readme-stats.vercel.app/api?username=hariomhub&show_icons=true&theme=tokyonight&hide_title=false&hide_border=false" />
  <img width="50%" src="https://github-readme-streak-stats.herokuapp.com/?user=hariomhub&theme=tokyonight&hide_border=false" />
</p>

<p align="center">
  <img width="30%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=hariomhub&layout=compact&theme=tokyonight&hide_border=false" />
</p>





